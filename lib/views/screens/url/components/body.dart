...
import 'package:secqrv3/views/screens/ml/predict_ml.dart';
                        ...
                        builder: (context, currentState) {
                          if (currentState is SucceedScanUrlState) {
                            urlCollection.add({
                              ...
                              'positives':
                                  '${currentState.urlScanReport.positives}',
                              'total': '${currentState.urlScanReport.total}',
                              // 'statusCode': currentState.urlScanReport.responseCode,
                              'scanDate': currentState.urlScanReport.scanDate,
                            ...
                            } else if (currentState.urlScanReport.positives !=
                                0) {
                          ...
                          BlocBuilder<UrlScanBloc, UrlScanState>(
                            builder: (context, currentState) {
                              if (currentState is SucceedScanUrlState) {
                                return TitleText(
                                    title:
                                        "${currentState.urlScanReport.positives} / ${currentState.urlScanReport.total}",
                                );
                              } else {
                                return TitleText(
                                    title:
                                        "0"); //Return there is no link inside the report yet
                              }
                            },
                          )
                        ],
                    ...
                    Padding(
                      ...
                      child: BlocBuilder<UrlScanBloc, UrlScanState>(
                        ...
                        if (currentstate is InitialScanUrlState) {
                          return OutputText(
                            text: "Please input your URL!",
                          );
                        }
                        if (currentstate is LoadingScanUrlState) {
                          return LoadingWidget(
                            imageloading: "assets/gif/spinner.gif",
                          );
                        }
                        if (currentstate is SucceedScanUrlState) {
                          if (currentstate.urlScan.isEmpty) {
                            return OutputText(
                                text: "Scan information is empty !");
                          }
                          return GridView.builder(
                              ...
                              itemBuilder: (context, index) => 
                                UrlScanCard(
                                  urlScan: currentstate.urlScan[index]));
                        }
                        if (currentstate is FailedScanUrlState) {
                          return OutputText(text: "Please check using the deep learning.",);
